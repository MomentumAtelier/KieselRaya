      ...
      {OPPORTUNITY_STAGES.map((stage) => {
        ...
        return (
          <div
            ...
            className={`w-72 shrink-0 rounded-2xl border transition-colors duration-300 ${
              dragOverStage === stage.value
                ? "border-bronze-500 bg-bronze-400/10"
                : "border-ink-900/[0.07] bg-ink-900/[0.02]"
            }`}
          >
            <div className="px-4 py-4 border-b border-ink-900/[0.07]">
            ...
            <div className="p-3 space-y-3 min-h-[120px] max-h-[65vh] overflow-y-auto">
              {stageOpps.map((opp) => (
                <KanbanCard
                  key={opp.id}
                  opportunity={opp}
